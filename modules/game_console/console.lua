...
function init()
  ...
  -- consolePanel = g_ui.displayUI('console', modules.game_interface.getBottomPanel())
  ...
  --g_keyboard.bindKeyPress('Alt+Tab', function() consoleTabBar:selectNextTab() end, consolePanel)
  ...
  g_keyboard.bindKeyPress('Ctrl+Enter', function()
  ...
  iconChat = modules.game_interface.getRootPanel():getChildById('iconChat')
  bottomSplitter = modules.game_interface.getRootPanel():getChildById('bottomSplitter')
...
function disableChat()
  ...
  consolePanel:show()
  gameInterface.getBottomPanel():show()
  bottomSplitter:enable()
  bottomSplitter:setMarginBottom(120)
  ...
  consoleToggleChat:setTooltip(tr("Disable chat mode, allow to walk using ASDW/QEZC\nPress Ctrl+Enter to disable\nIf the chat is closed, you can attack the monsters with the TAB key"))
end
...
function enableChat()
  ...
  gameInterface.getBottomPanel():hide()
  consolePanel:hide()
  bottomSplitter:disable()
  bottomSplitter:setMarginBottom(-2)
  ...
  g_keyboard.bindKeyPress('Ctrl+W', function() g_game.turn(North)  end)
  g_keyboard.bindKeyPress('Ctrl+D', function() g_game.turn(East)   end)
  g_keyboard.bindKeyPress('Ctrl+S', function() g_game.turn(South)  end)
  g_keyboard.bindKeyPress('Ctrl+A', function() g_game.turn(West)   end)
...
if string.sub(text, 0, 21) == ": Message of the Day:" then
	text = string.sub(text, 22, -1)
...
function sendCurrentMessage()
  ...
  --Modulo de spells-- Mirto
  --modules.game_textmessage.displayGameMessage("Player does not exist or is not in a range.")
end
...
function onTalk(name, level, mode, message, channelId, creaturePos)
  ...
  elseif speaktype.private then
	...
	if ( not consolePanel:isVisible() ) then
		iconChat:show()
		g_effects.startBlink(iconChat, 0, 1000)
	end
...
function chooseTarget()
  ...
  for i, creature in pairs(spectators) do
    if ( getDistanceBetween(g_game.getLocalPlayer():getPosition(), creature:getPosition()) <= distanceTarget ) then
      if ( isMonsterOnTarget(creature) ) then
        if ( monstersAttacked[creature:getId()] ) then
          if ( monstersAttacked[creature:getId()] ~= 1 ) then
            g_game.attack(creature)
            break
          end
        else
          monstersAttacked[creature:getId()] = 1
          g_game.attack(creature)
          break
	    end
      end
    end
    if ( i == #spectators ) and ( monstersAttacked ~= nil ) and ( next(monstersAttacked) ~= nil ) then
      monstersAttacked = nil
      monstersAttacked = {}
      chooseTarget()
    end
  end
...
function isMonsterOnTarget(creatureData)
  if creatureData:isMonster() and not creatureData:isInvisible() then
	return true
...
function online()
  ...
  disableChat()
  iconChat:hide()
  folds = rootWidget:recursiveGetChildById('spellBarPanel')
  folds:show()
  folds:setVisible(true)
...
function toggleSpellBar()
    if folds:isVisible() then
       folds:setVisible(false)
    else
       folds:setVisible(true)
    end
end
