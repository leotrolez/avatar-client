...
function onGameEditText(id, itemId, maxLength, text, writter, time)
  if ( text:lower():find('task') ) then
    onTaskWindow(text, maxLength)
	return 
  end
  ...
  if not writeable then
    textWindow:setText(tr('Information:'))
    cancelButton:hide()
...
function onGameEditList(id, doorId, text)
  local textWindow = g_ui.createWidget('TextWindow2', rootWidget)
...
function onTaskWindow(tasks, maxLength)
  local taskWindow = g_ui.createWidget('TaskWindow', rootWidget)
  local textEdit = taskWindow:getChildById('text')
  local closeButton = taskWindow:getChildById('closeButton')
  
  textEdit:setText(tasks)
  textEdit:setMaxLength(maxLength)
  
  local function destroy()
    taskWindow:destroy()
    table.removevalue(windows, taskWindow)
  end
  
  closeButton.onClick = destroy
  taskWindow.onEscape = destroy
  table.insert(windows, taskWindow)
end
