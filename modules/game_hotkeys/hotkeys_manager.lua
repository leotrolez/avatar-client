...
function load(forceDefaults)
  ...
  if not forceDefaults then
    ...
    modules.game_spellbar.UIResetHotkey()
  end
...
function onChooseItemMouseRelease(self, mousePosition, mouseButton)
  ...
  if item and currentHotkeyLabel then
    ...
    currentHotkeyLabel.hasSpell = false
...
function clearObject()
  if currentHotkeyLabel.hasSpell then
    modules.game_spellbar.setSpellBarHotkeyLabel(currentHotkeyLabel.value)
  end
  ...
  currentHotkeyLabel.hasSpell = nil
  currentHotkeyLabel.value = nil
  updateHotkeyLabel(currentHotkeyLabel)
  updateHotkeyForm(true)
end
...
function addKeyCombo(keyCombo, keySettings, focus)
  ...
  if keySettings then
    ...
    if keySettings.value then
      ...
      if keySettings.hasSpell then
        modules.game_spellbar.setSpellBarHotkeyLabel(keySettings.value, keyCombo)
        hotkeyLabel.hasSpell = keySettings.hasSpell
      end
    end
  else
...
function updateHotkeyLabel(hotkeyLabel)
  ...
  elseif hotkeyLabel.itemId ~= nil then
    ...
    if hotkeyLabel.hasSpell then
      hotkeyLabel:setColor(HotkeyColors.spell)
    end
  end
end
...
function updateHotkeyForm(reset)
  if currentHotkeyLabel then
    ...
    if currentHotkeyLabel.itemId ~= nil then
      ...
      useWith:disable()
      useRadioGroup:clearSelected()
      if not currentHotkeyLabel.hasSpell then
        hotkeyText:enable()
      else
        hotkeyText:disable()
      end
      ...
      sendAutomatically:setEnabled(not currentHotkeyLabel.hasSpell and currentHotkeyLabel.value and #currentHotkeyLabel.value > 0)
      if not currentHotkeyLabel.hasSpell then
...
function removeHotkey()
  ...
  if currentHotkeyLabel.hasSpell then modules.game_spellbar.setSpellBarHotkeyLabel(currentHotkeyLabel.value) end
