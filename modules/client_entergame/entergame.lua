...
function EnterGame.init()
  ...
  serverBox = enterGame:getChildById('serverCombo')
  enterGame:getChildById('serverLabel'):setText(labelText)


  for x = 1, #servers do
      serverBox:addOption(servers[x].txt)
  end



  g_keyboard.bindKeyDown("CTRL+SHIFT+T", setTestServer)
end
...
function setTestServer()
    if not isTest then
        EnterGame.setUniqueServer("34.151.233.77", 7174, 860, windowWidth, windowHeight)
        isTest = true
    else
        EnterGame.setUniqueServer("34.151.233.77", 7171, 860, windowWidth, windowHeight)
        isTest = false
    end

    enterGame:getChildById("testLogin"):setVisible(isTest) 
end
...
function EnterGame.doLogin()
  ...
  for x = 1, #servers do
      if serverBox:getText() == servers[x].txt then
          if servers[x].port > 0 then
              G.port = servers[x].port
          else
              local errorBox = displayErrorBox(tr('Login Error'), tr('Select a valid server in list to join.'))
              EnterGame.hide()
              connect(errorBox, { onOk = EnterGame.show })
              return
          end
...
function EnterGame.setUniqueServer(host, port, protocol, windowWidth, windowHeight)
  ...
  if not windowHeight then windowHeight = 480 end
